...
/*
  if (defined('')) {
    $db->Execute("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key IN ('GOOGLE_SITEMAP_PROD_CHANGE_PRIOR', 'GOOGLE_SITEMAP_CAT_CHANGE_PRIOR', 'GOOGLE_SITEMAP_EZPAGES_CHANGE_PRIOR', 'GOOGLE_SITEMAP_USE_ROOT_DIRECTORY', 'GOOGLE_SITEMAP_XML_FS_DIRECTORY', );");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_COMPRESS'
                  WHERE configuration_key='SITEMAPXML_COMPRESS'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_LASTMOD_FORMAT'
                  WHERE configuration_key='SITEMAPXML_LASTMOD_FORMAT'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_USE_XSL'
                  WHERE configuration_key='SITEMAPXML_USE_XSL'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_PROD_CHANGE_FREQ'
                  WHERE configuration_key='SITEMAPXML_PROD_CHANGE_FREQ'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_CAT_CHANGE_FREQ'
                  WHERE configuration_key='SITEMAPXML_CAT_CHANGE_FREQ'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_EZPAGES_CHANGE_FREQ'
                  WHERE configuration_key='SITEMAPXML_EZPAGES_CHANGE_FREQ'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_EZPAGES_HEADER'
                  WHERE configuration_key='SITEMAPXML_EZPAGES_HEADER'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_EZPAGES_SIDEBOX'
                  WHERE configuration_key='SITEMAPXML_EZPAGES_SIDEBOX'");
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " 
                  SET configuration_key='GOOGLE_SITEMAP_EZPAGES_FOOTER'
                  WHERE configuration_key='SITEMAPXML_EZPAGES_FOOTER'");
  }
*/
?>
          ...
          <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="main">
            <tr>
              <td width="78%" align="left" valign="top">
                ...
                <?php echo sprintf(TEXT_SITEMAPXML_INSTRUCTIONS_STEP1, "\"javascript:(void 0)\" class=\"splitPageLink\" onClick=\"javascript:window.open('" . zen_catalog_href_link(FILENAME_SITEMAPXML) . "', 'sitemapGen', 'resizable=1,statusbar=5,width=700,height=400,top=0,left=50,scrollbars=yes')\""); ?>
                <?php echo sprintf(TEXT_SITEMAPXML_INSTRUCTIONS_STEP2, "\"javascript:(void 0)\" class=\"splitPageLink\" onClick=\"javascript:window.open('" . zen_catalog_href_link(FILENAME_SITEMAPXML, 'genxml=no&ping=yes') . "', 'sitemapGen', 'resizable=1,statusbar=5,width=600,height=400,top=0,left=50,scrollbars=yes')\""); ?>
<!--
                <fieldset>
