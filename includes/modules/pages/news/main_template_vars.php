...
//$newsDate = strtoupper(zen_date_long($date));
  ...
  if ($news->RecordCount() > 0) {
    ...
    $article_array = array();
    while (!$news->EOF) {
      $comments = $db->Execute("select count(*) as count from " . TABLE_NEWS_COMMENTS . " where approved = '1' and article_id = '" . (int)$news->fields['article_id'] . "'");
      ...
      if ((zen_not_null($news->fields['news_image_two'])) && file_exists(DIR_FS_CATALOG . DIR_WS_IMAGES . $news->fields['news_image_two'])) {
        $article_array[$i]['articleImage'] = zen_image(DIR_WS_IMAGES . $news->fields['news_image_two'], $news->fields['news_image_text_two'], '', '', 'align="right" class="articleImage"');
      }
    ...
    // Article Links
      ...
      $article_array[$i]['links'][0]['offsiteLinks'] = $offsite_links_array;
      ...
      $article_array[$i]['links'][0]['productLinks'] = $product_links_array;
      ...
      $article_array[$i]['links'][0]['storeLinks'] = $store_links_array;

			$i++;
			$news->MoveNext();
    }

    $newsArticlesBlock = $article_array;
  } else {
    $articlesNotFound = true;
	  $newsCurrentDate = zen_date_long($curren_date);
  }
