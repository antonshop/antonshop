...
//  Store Transaction history in Database
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('启用FirstData/Linkpoint支付模块', 'MODULE_PAYMENT_LINKPOINT_API_STATUS', 'True', '是否启用FirstData/Linkpoint信用卡支付?', '6', 121, 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('FirstData/Linkpoint/YourPay商户编号', 'MODULE_PAYMENT_LINKPOINT_API_LOGIN', 'EnterYourStoreNumber', '请输入您的FirstData/Linkpoint/YourPay商户编号。<br />与您帐号的PEM数字证书文件名中的数字一样。', '6', 121, now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('FirstData/Linkpoint交易模式响应', 'MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE_RESPONSE', 'LIVE: Production', '<strong>正式:</strong> 正式运营的网店使用。<br />如果要测试模块，选择这些选项:<br /><strong>Successful:</strong> 用于测试一个成功的交易<br /><strong>Decline:</strong> 用于测试一个失败的交易', '6', 121, 'zen_cfg_select_option(array(\'LIVE: Production\', \'TESTING: Successful\', \'TESTING: Decline\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('授权类型', 'MODULE_PAYMENT_LINKPOINT_API_AUTHORIZATION_MODE', 'Authorize Only', '您只要授权提交的信用卡，还是要授权并接受?<br />通常您可以<strong>Immediate Charge</strong>立刻接受。有时采用<strong>Authorize</strong>交易，然后手工通过自己的商户终端交易 (特别是运费不确定时)', '6', 121, 'zen_cfg_select_option(array(\'Authorize Only\', \'Immediate Charge/Capture\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('订单状态', 'MODULE_PAYMENT_LINKPOINT_API_ORDER_STATUS_ID', 2, '设置通过该支付方式付款的订单状态<br />(影响所有的已收款/已支付/已批准的订单)<br />建议: <strong>处理中</strong>', '6', 121, 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('预授权订单状态', 'MODULE_PAYMENT_LINKPOINT_API_PREAUTH_ORDER_STATUS_ID', 1, '如果设置为PREAUTH模式(授权), 订单状态设置为?<br />推荐: <strong>Pending</strong>', '6', 121, 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('退款/失效订单状态', 'MODULE_PAYMENT_LINKPOINT_API_REFUNDED_ORDER_STATUS_ID', '1', '如果有退款或失效订单，订单状态设置为?<br />推荐: <strong>Pending</strong> 或 取消/退款', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('显示顺序', 'MODULE_PAYMENT_LINKPOINT_API_SORT_ORDER', '0', '显示顺序：小的显示在前。', '6', 121, now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('付款地区', 'MODULE_PAYMENT_LINKPOINT_API_ZONE', '0', '如果选择了付款地区，仅该地区可以使用该支付模块。', '6', 121, 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('欺诈报警', 'MODULE_PAYMENT_LINKPOINT_API_FRAUD_ALERT', 'Yes', '有可疑信用卡交易是，是否要通过电子邮件通知店主?', '6', 121, 'zen_cfg_select_option(array(\'Yes\', \'No\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('启用数据库存储', 'MODULE_PAYMENT_LINKPOINT_API_STORE_DATA', 'True', '保存网关通信数据到数据库中。在 管理页面->客户管理->FirstData/LinkPoint 信用卡记录中查看。', '6', 121, 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('调试模式', 'MODULE_PAYMENT_LINKPOINT_API_DEBUG', 'Off', '要启用调试模式吗? 详细的错误报告将发送到店主的邮箱。', '6', '0', 'zen_cfg_select_option(array(\'Off\', \'Failure Alerts Only\', \'Log File\', \'Log and Email\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('交易模式', 'MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE', 'Production', '用于处理订单的交易模式', '6', 121, 'zen_cfg_select_option(array(\'Production\',\'DevelopersTest\'), ', now())");
