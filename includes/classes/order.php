...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
	...
	else
	{
		...
		$to_address=$this->customer['email_address'];
		$email_subject=EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id;
		$email_text=$email_order;
		$from_email_name=STORE_NAME;
		$from_email_address=EMAIL_FROM;
		$block=$html_msg;
		
		$sql = "insert into  95EPAY_EMAIL(to_name, to_address, email_subject, email_text, from_email_name, from_email_address, block,real_order_no) values ('" .zen_db_input($to_name) . "','".zen_db_input($to_address). "','" . zen_db_input($email_subject) ."','" . zen_db_input($email_text) . "','". zen_db_input($from_email_name). "','". zen_db_input($from_email_address) . "','" . base64_encode(serialize($block)) . "','" . $zf_insert_id . "')";
		$db->Execute($sql);
	}
	

    // send additional emails
    if (SEND_EXTRA_ORDER_EMAILS_TO != '' && $_SESSION['payment']!="_95epay") {
	...
	else if (SEND_EXTRA_ORDER_EMAILS_TO != '' && $_SESSION['payment']=="_95epay") {
		 $extra_info=email_collect_extra_info('','', $this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], $this->customer['telephone']);
      $html_msg['EXTRA_INFO'] = $extra_info['HTML'];
	  ...
	  $to_address=SEND_EXTRA_ORDER_EMAILS_TO;
		$email_subject=SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id;
		$email_text=$email_order . $extra_info['TEXT'];
		$from_email_name=STORE_NAME;
		$from_email_address=EMAIL_FROM;
		$block=$html_msg;
		$sql = "insert into  95EPAY_EXTRA_EMAIL(to_address, email_subject, email_text, from_email_name, from_email_address, block,real_order_no) values ('".zen_db_input($to_address). "','" . zen_db_input($email_subject) ."','" . base64_encode(serialize($email_text)) . "','". zen_db_input($from_email_name). "','". zen_db_input($from_email_address) . "','" . base64_encode(serialize($block)) . "','" . $zf_insert_id . "')";
		$db->Execute($sql);
