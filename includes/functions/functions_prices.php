...
/* only use new New Products and Specials */
/* edit this function please call zhangchaofeng */
  function zen_get_products_display_price_sam($products_id) {
...
// show case only
    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      ...
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          ...
          $show_sale_discount = '<dd><span class="pro_inf">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span></dd>';

        } else {
          $show_sale_discount = '<dd><span class="pro_inf">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span></dd>';
        }
      } else {
        if (SHOW_SALE_DISCOUNT == 1) {
          $show_sale_discount = '<dd><span class="pro_inf">' . PRODUCT_PRICE_DISCOUNT_PREFIX . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span></dd>';
        } else {
          $show_sale_discount = '<dd><span class="pro_inf">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span></dd>';
        }
      }
    ...
    if ($display_special_price) {
      ...
      $show_normal_price = '<dd><var>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </var>';
	  
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        $show_special_price = '<em>' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</em>';
        if ($product_check->fields['product_is_free'] == '1') {
          $show_sale_price = '<em>' . PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</em>';
        } else {
          $show_sale_price = '<em>' . PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</em>';
        }
      } else {
      ...
      if ($display_sale_price) {
        $show_normal_price = '<dd><var>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </var>';
        $show_special_price = '';
        $show_sale_price = '<em>'. PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</em></dd>';
      } else {
        if ($product_check->fields['product_is_free'] == '1') {
          $show_normal_price =  '<dd><var>' .$currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </var>';
        } else {
          $show_normal_price = '<dd><em>' .$currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])). ' </em>';
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
...
/* only use products content */
/* edit this function please call zhangchaofeng */
  function zen_get_products_display_price_content($products_id) {
    global $db, $currencies;
    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      ...
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
		  ...
		  $price_info['sale_discount'] = PRODUCT_PRICE_DISCOUNT_PREFIX . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE;

        } else {
		  $price_info['sale_discount'] = PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT;
        }
      } else {
        if (SHOW_SALE_DISCOUNT == 1) {
		  $price_info['sale_discount'] =  PRODUCT_PRICE_DISCOUNT_PREFIX . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE;
        } else {
		  $price_info['sale_discount'] = PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT;
        }
      }
    
	}
    if ($display_special_price) {
	  $price_info['normal_price'] = $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id']));
	  
      if ($display_sale_price && $display_sale_price != $display_special_price) {
		$price_info['special_price'] = $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id']));
        if ($product_check->fields['product_is_free'] == '1') {
		  $price_info['sale_price']  = PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id']));
        } else {
		  $price_info['sale_price'] =  PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id']));
        }
      } else {
    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          ...
          $show_sale_discount = '<br><span class="save_price"><em>'  . PRODUCT_PRICE_DISCOUNT_PREFIX . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</em></span>';

        } else {
          $show_sale_discount = '<br><span class="save_price"><em>'  . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</em></span>';
        }
      } else {
        if (SHOW_SALE_DISCOUNT == 1) {
          $show_sale_discount = '<br><span class="save_price"><em>'  . PRODUCT_PRICE_DISCOUNT_PREFIX . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</em></span>';
        } else {
          $show_sale_discount = '<br><span class="save_price"><em>'  . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</em></span>';
        }
      }
    }

    if ($display_special_price) {
      $show_normal_price = '<span class="price">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
      if ($display_sale_price && $display_sale_price != $display_special_price) {
		  
        $show_special_price = '&nbsp;' . '<span class="normalprice">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
        if ($product_check->fields['product_is_free'] == '1') {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_special_price = '&nbsp;' . '<span class="normalprice">' . '<s>' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</span>';
		  
        } else {
          $show_special_price = '&nbsp;' . '<span class="normalprice">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
      ...
      if ($display_sale_price) {
        $show_normal_price = '<span class="price">Price:<var>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</var> </span>';
        $show_special_price = '';
        $show_sale_price = '<br />' . '<span class="productSalePrice">' . PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
      } else {
        if ($product_check->fields['product_is_free'] == '1') {
          $show_normal_price = 'Price:<s>' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>';
        } else {
          $show_normal_price = '<span class="price">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])).'</span>';
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
